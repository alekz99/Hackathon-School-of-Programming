...
class ASRDiarizationPipeline:
    def __init__(
        ...
        self.asr_array = asr_array
        self.diar_array = diar_array
    ...
    def __call__(
        ...
        **kwargs,
        ...
        metadata = torchaudio.info(self.init_inputs)
        audio_end = round(metadata.num_frames / metadata.sample_rate, 1)
        ...
        #segments = []
        segments = self.diar_array[index]
        ...
        if len(segments) == 0:
          segments.append({'segment': {'start': 0.0, 'end': audio_end}, 'track': 'A', 'label': 'SPEAKER_00'})
        new_segments = []
        ...
        #    **kwargs_asr,
        #)
        asr_out = self.asr_array[index]
        transcript = asr_out["chunks"]

        # get the end timestamps for each chunk from the ASR output
        end_timestamps = np.array([audio_end if chunk["timestamp"][-1] is None else chunk["timestamp"][-1]  for chunk in transcript])
        segmented_preds = []
